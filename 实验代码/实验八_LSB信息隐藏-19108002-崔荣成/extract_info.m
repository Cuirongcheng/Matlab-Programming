function bitword=extract_info(varargin)
...
%uscita (la stringa appunto). Pu?accettare un'eventuale key numerica 
...
if length(varargin)==0                          %Controllo il numero di parametri passati in input
    return
elseif length(varargin)==2                      %Se ?stato specificato un secondo parametro:
...
while (i<=length(im))&&(flag==0),               %Finquando non analizzo tutta l'immagine e non trovo il carattere tappo:
    ...
    if(bitchar==255)                            %Controllo se il carattere letto ?il tappo
