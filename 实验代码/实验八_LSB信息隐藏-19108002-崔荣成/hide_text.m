function varargout=hide_text(varargin)
...
if length(varargin)<2           %Controllo il numero di parametri passati in input
    return
elseif length(varargin)==3      %Se ?stato specificato un terzo parametro:
...
if (length(stringa)+1)<=(length(im)/8)              %Controllo se la dimensione dell'immagine ?sufficiente a ospitare la stringa
    ...
    if length(varargin)==3      %Controllo se ?presente una key:
    ...
    for i=1:length(stringa)     %Per ogni carattere della stringa:
        for j=1:8                                   %Per ogni bit del singolo carattere:
            if length(varargin)==3                  %Controllo se ?presenre una key:
                index=k((i-1)*8 + j);               %Calcolo l'indice del pixel da modificare
            else                                    %Se non ?presente la key:
...
else                                                %Nel caso la dimensione dell'immagine non ?sufficiente a contenere "stringa":
